...
function App() {
  ...
  const addRoom = () => {
    ...
    const newRect = {
      x,
      y,
      width: 100, // Room: wider
      height: 50, // Room: shorter
      fill: 'red', // Room: red
      id: `rect${idCounter.current}`,
      type: 'room', // Fixed type
    };
  ...
  const addHallway = () => {
    const maxX = canvasWidth - 50; // Adjusted for hallway width
    const maxY = canvasHeight - 100; // Adjusted for hallway height
    const x = Math.round((Math.random() * maxX) / gridSize) * gridSize;
    const y = Math.round((Math.random() * maxY) / gridSize) * gridSize;
    idCounter.current += 1;
    const newRect = {
      x,
      y,
      width: 50, // Hallway: narrower
      height: 100, // Hallway: taller
      fill: 'blue', // Hallway: blue
      id: `rect${idCounter.current}`,
      type: 'hallway', // Fixed type
    };
    setRectangles([...rectangles, newRect]);
    setSelectedId(null);
    trRef.current.nodes([]);
